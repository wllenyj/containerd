...
func (cm *criManager) Run() error {
	...
	if err := cm.recover(ctrdutil.NamespacedContext()); err != nil {
		return errors.Wrap(err, "failed to recover state")
	}

	// run services
	for mode, s := range cm.services {
...
func (cm *criManager) getServiceByRuntime(runtime string) (GrpcServices, error) {
	...
	if len(r.Mode) == 0 || r.Mode == criconfig.RuntimeModeDefault {
		return cm.c, nil
	}
	return cm.getServiceByMode(r.Mode)
}

func (cm *criManager) getServiceByMode(mode string) (GrpcServices, error) {
	i, ok := cm.services[mode]
	if !ok {
		// there is not plugin
...
func (cm *criManager) getServiceBySandboxID(id string) (GrpcServices, error) {
	...
	if len(sandbox.GetMetadata().Mode) == 0 || sandbox.GetMetadata().Mode == criconfig.RuntimeModeDefault {
		return cm.c, nil
	}
	return cm.getServiceByMode(sandbox.GetMetadata().Mode)
}
