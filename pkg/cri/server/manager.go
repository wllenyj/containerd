...
func (cm *criManager) getServiceByMode(mode string) (GrpcServices, error) {
	...
	if !ok {
		// there is not plugin
		return nil, errors.Errorf("the mode %q not implement plugin", mode)
	}
	return i, nil
}
...
func (cm *criManager) getServiceBySandboxID(id string) (GrpcServices, error) {
	...
	if len(sandbox.Mode) == 0 || sandbox.Mode == criconfig.RuntimeModeDefault {
		return cm.c, nil
	}
	return cm.getServiceByMode(sandbox.Mode)
}
